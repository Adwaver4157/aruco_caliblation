...
def generateArMarker(size=3):
	...
	for i in range(cnt):
		...
		# 右端まで達するまで水平用bufferにconcat
			...
			if (i % size == size-1 and i // size == 0):
				vconcat_img = hconcat_img

			# 右端まで行きかつ２段目以降の時は鉛直方向にconcat
			elif (i % size == size-1 and i // size > 0):
	...
	# 結合したArUcoマーカを出力
	filename = f"{size**2}_qr.png"
	file_path = os.path.join(image_dir, filename)
	cv2.imwrite(file_path, vconcat_img)
        
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Making aruco marker")
	parser.add_argument("--size", type=int, default=3)

	args = parser.parse_args()
	generateArMarker(args.size)